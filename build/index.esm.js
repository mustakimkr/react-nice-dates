...
function DateRangePickerCalendar(_ref) {
  ...
  var modifiers = mergeModifiers({
    ...
    disabled: function disabled(date) {
      return focus === START_DATE && endDate && (differenceInDays(startOfDay(endDate), date) < minimumLength && !startDate || !startDate && maximumLength && differenceInDays(startOfDay(endDate), date) > maximumLength) || focus === END_DATE && startDate && (differenceInDays(date, startOfDay(startDate)) < minimumLength && !endDate || !endDate && maximumLength && differenceInDays(date, startOfDay(startDate)) > maximumLength);
    }
  }, receivedModifiers);
